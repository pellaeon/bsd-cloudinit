...
class FreeBSDUtils(base.BaseOSUtils):
    ...
    def set_portsnap_server(self, server):
        """
        This function will modify the 'SERVERNAME' variable in /etc/portsnap.conf .
        param server: string
        """
        config_file = '/etc/portsnap.conf'
        if !os.path.isfile( config_file ) :
            LOG.error( config_file + ': not a regular file.')
            return

        cmd = 'sed -e \'s/^SERVERNAME=/# &/g\' -I ".bak" ' + config_file
        subprocess.check_call(cmd, shell=True)

        file_obj = open(config_file, 'a')
        file_obj.write('\n')
        self._add_comment(file_obj)
        file_obj.write('SERVERNAME=' + server)
        file_obj.close()
